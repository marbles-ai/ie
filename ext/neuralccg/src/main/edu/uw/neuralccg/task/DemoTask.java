...
public class DemoTask implements ITask<Any> {
    ...
    @Override
    public Stream<Any> run(Config parameters) {
        final File modelDir = new File(parameters.getString("model_dir"));

        final File checkpointPath = new File(parameters.getString("checkpoint_path"));
