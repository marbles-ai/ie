...
class AstWalker(NodeVisitor):
    ...
    __argsRE = regexpCompile(r"^\s*(?P<name>\w+)\s*(?P<type>\(?\S*\)?)?\s*"
                             r"(?:-|:)+\s+(?P<desc>.+)$")\
    # PWG: Allow remarks
    __returnsStartRE = regexpCompile(r"^\s*(Return|Yield|Remark)s:\s*$", IGNORECASE)
    __raisesStartRE = regexpCompile(r"^\s*(Raises|Exceptions|See Also):\s*$",
                                    IGNORECASE)
    __listRE = regexpCompile(r"^\s*(([\w\.]+),\s*)+(&|and)?\s*([\w\.]+)$")
    # Allow functions in See Also list
        ...
        while True:
            ...
            if line is not None:
                ...
                if self.options.autobrief:
                    ...
                    if match:
                        # PWG: remarks look the same so reuse logic
                        # We've got a "returns" section
                        if 'remark' in match.group(1).lower():
                            line = line.replace(match.group(0), ' @remark\t').rstrip()
                            prefix = '@remark\t'
                        else:
    ...
    def visit_Module(self, node, **kwargs):
        ...
        if self.options.debug:
            stderr.write("# Module {0}{1}".format(self.options.fullPathNamespace, linesep))
    ...
    def visit_Assign(self, node, **kwargs):
        ...
        if match:
            ...
            if self.options.debug:
                stderr.write("# Attribute {0.id}{1}".format(node.targets[0], linesep))
    ...
    def visit_Call(self, node, **kwargs):
        ...
        if match:
            self.lines[lineNum] = '{0}## @implements {1}{2}{0}{3}{2}'.format(
                ...
                self.lines[lineNum].rstrip())
            if self.options.debug:
                stderr.write("# Implements {0}{1}".format(match.group(1), linesep))
        # Visit any contained nodes.
        self.generic_visit(node, containingNodes=kwargs['containingNodes'])
